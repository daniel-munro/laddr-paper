...
pantry <- tibble(modality = c("expression", "isoforms", "splicing", "alt_TSS", "alt_polyA", "stability")) |>
    reframe(
        read_tsv(
            str_glue("data/pantry/geuvadis/phenotypes/{modality}.bed.gz"),
            col_types = cols(`#chr` = "-", start = "-", end = "-", phenotype_id = "c", .default = "d")
        ),
        .by = modality
    ) |>
    mutate(gene_id = str_extract(phenotype_id, "^[^:.]+"), .before = phenotype_id) |>
...
latent_full <- read_tsv(
    "data/residual/latent-full.bed.gz",
    col_types = cols(`#chr` = "-", start = "-", end = "-", phenotype_id = "c", .default = "d")
) |>
    separate_wider_delim(phenotype_id, ":", names = c("gene_id", "phenotype_id")) |>
...
latent_residual <- read_tsv(
    "data/residual/latent-residual.bed.gz",
    col_types = cols(`#chr` = "-", start = "-", end = "-", phenotype_id = "c", .default = "d")
) |>
    separate_wider_delim(phenotype_id, ":", names = c("gene_id", "phenotype_id")) |>
    mutate(modality = "latent", .before = 1)
