...
# "Applied to GTEx, LaDDR identified an average of 95% more independent xQTLs per tissue than the six transcriptional regulation modes implemented in Pantry."
...
####################################################################
## Results: LaDDR phenotypes capture familiar and novel variation ##
####################################################################
...
##############################################################################
## Results: LaDDR phenotypes encode multimodal genetic regulatory variation ##
##############################################################################
...
# "finding that, on average, 95% more independent cis-QTLs were identified for DDPs than for KDPs per tissue"
...
####################################################################
## LaDDR phenotypes can complement knowledge-driven phenotypes... ##
####################################################################

# "and examined the maximum Pearson r2 between each residual latent phenotype and an explicit phenotype of the same gene. For PC1 DDPs/rDDPs, residualization reduced these values from mean 0.40 (SD 0.27) to mean 0.068 (SD 0.093) (Figure 4a)."
...
# "Mapping KDPs resulted in 22,470 total xQTLs, while mapping DDPs resulted in 30,158 xQTLs.
# Adding rDDPs to the set of KDPs resulted in a net increase of 9,408 xQTLs (42%) for a total of 31,878, 16,544 of which were called for KDPs and 15,334 of which were called for rDDPs"
...
# "In terms of unique genes represented by the xQTLs, addition of the rDDPs increased the number of xGenes from 12,872 to 16,950"
...
# "Despite long non-coding RNAs (lncRNAs) having higher transcript complexity (i.e., splice variants per exon) than protein-coding mRNAs, xQTL mapping across six KDP modalities produced an average of 0.16 independent xQTLs per gene per tissue for lncRNAs, which is only 29% of the rate for protein-coding genes."
...
# "In the annotations used in this study, lncRNAs have 28.9% the number of exons per gene summed over isoforms compared to protein-coding genes."
...
# "However, DDP xQTL mapping showed less of a difference, with 0.39 xQTLs per lncRNA per tissue, which was 44% of the rate for protein-coding genes."
...
###########################################
## Results: Impact of input data quality ##
###########################################
...
# Unexpectedly, using 54 rather than five tissues to fit models increased the number of independent xQTLs per tissue by only 4.2% on average
# Using 54 tissues plus 33 cancer types increased xQTLs by 1.7% on average compared to 54 tissues only
...
# "DDPs were less affected by gene annotation sparsity than were KDPs, with independent xQTLs dropping 1.8% between 100% and 0% inclusion of non-canonical isoforms, compared to a 14% drop for explicit KDPs"
...
# "When we used each of these variations as input for LaDDR and mapped cis-QTLs to the DDPs, we observed a relatively small drop (-2.7%) in discoveries from the 50 bp truncated reads, and larger drops from the single-end (-7.2%) and lowered sequencing depth (-41% for 50% of reads, -66% for 25% of reads) simulations"
